...
// Before any command can reach the network: silence needle's DEP0169 noise and
// nothing else. See lib/filter-warnings.js.
require("./filter-warnings").filterWarnings();
...
const commands = {
  ...
  uninstall: {
    ...
    args: "<version>",
    exec: parsed => require("./uninstall")(parsed.args.version, parsed.opts)
  },
  use: {
    desc: "use the given version of Node.js in current shell",
    args: "[version]",
    exec: parsed => require("./use")(parsed.args.version)
  },
  "auto-use": {
    ...
    exec: async parsed => {
      ...
      if (action === "enable") {
        await require("./enable-auto-use").enableAutoUse(useCdWrapper);
      } else if (action === "disable") {
        await require("./enable-auto-use").disableAutoUse();
      } else if (action) {
        // Unknown action
  ...
  stop: {
    ...
    alias: "unuse",
    exec: () => require("./deactivate")()
  },
  link: {
    desc: "permanently link the version of Node.js as default (supports 'lts' and 'latest')",
    args: "<version>",
    exec: parsed => {
      // "lts" resolves against the remote version list, so link needs the same
      // proxy/ssl handling as install and ls-remote. Resolved through checkOpts
      // so option precedence stays defined in exactly one place.
      const { proxy, proxySource, verifyssl } = checkOpts(parsed);
      return require("./switch")(parsed.args.version, { proxy, verifyssl, proxySource });
    }
  },
  unlink: {
    desc: "permanently unlink the default version",
    exec: () => require("./switch-deactivate")()
  },
  ls: {
    desc: "list all the installed Node.js versions",
    exec: () => require("./ls").local()
  },
  "ls-remote": {
    ...
    exec: parsed => {
      const { proxy, proxySource, verifyssl } = checkOpts(parsed);
      common.logProxyInfo(proxy, proxySource);
      return require("./ls").remote(proxy, verifyssl);
    }
  },
  cleanup: {
    desc: "remove stale local caches",
    exec: () => require("../lib/cleanup")()
  },
  postinstall: {
    desc: "Invoke custom post install script for the given version",
    args: "[version]",
    exec: parsed => require("../lib/post-install")(parsed.args.version)
  },
  "init-env": {
...
`);
  ...
  .parseAsync()
  .catch(err => {
    common.log(ck`<red>nvm command failed</>`, err);
    common.exit(1);
  });

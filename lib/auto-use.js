...
async function autoUse(options = {}) {
  ...
  try {
    ...
    let version, nodeDir;
    try {
      const result = await common.findNodeVersion(cleanVer, false, true);
      ...
      if (!hasShownError(errorKey)) {
        common.log(ck`<yellow>Universal NVM: Version ${common.ckEscape(cleanVer)} from ${source} not installed</>`);
    ...
    const rangeInfo = range ? `: ${common.ckEscape(range)}` : "";
    common.log(ck`<green>Universal NVM: ${action} node ${version}</> <dim>(${prefix} ${source}${rangeInfo})</>`);

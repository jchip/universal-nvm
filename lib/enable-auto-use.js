...
const nvmHome = process.env.NVM_HOME || path.join(homeDir, ".unvm");
...
function detectShellType() {
  ...
  // - 'powershell' or 'cmd' when called from nvm.ps1/nvm.cmd (Windows/macOS/Linux)
  ...
  // Check for PowerShell environment variables (works on Windows, macOS, and Linux)
  if (process.env.NVM_POWERSHELL || process.env.PSModulePath) {
    return "powershell";
  }
  ...
  // Detect bash vs zsh from environment on Unix
...
function getPowerShellProfilePath() {
  ...
  // On Unix (macOS/Linux), PowerShell 7 uses ~/.config/powershell/
  const platform = os.platform();
  if (platform !== "win32") {
    return path.join(homeDir, ".config", "powershell", "Microsoft.PowerShell_profile.ps1");
  }

  // On Windows, check for PowerShell 7+ profile first
  ...
  // Fall back to PowerShell 5 profile on Windows
...
function enableAutoUsePowerShell(useCdWrapper = false) {
  ...
  // Use forward slashes on Unix, backslashes on Windows
  const isUnix = os.platform() !== "win32";
  const pathSep = isUnix ? "/" : "\\";

  // Platform detection and path construction for PowerShell
  const setupLines = isUnix ? [
    BEGIN_MARKER,
    `$IsUnix = $PSVersionTable.PSVersion.Major -ge 6 -and ($IsLinux -or $IsMacOS)`,
    `$nvmAutoUsePath = if ($IsUnix) { "${nvmHomeVar}${pathSep}bin${pathSep}nvm-auto-use.ps1" } else { "${nvmHomeVar}\\bin\\nvm-auto-use.ps1" }`,
    `. $nvmAutoUsePath`,
    enableCommand,
    END_MARKER
  ] : [
    BEGIN_MARKER,
    `. "${nvmHomeVar}${pathSep}bin${pathSep}nvm-auto-use.ps1"`,
    enableCommand,
    END_MARKER
  ];
  ...
  // Write back (use platform-specific line endings)
  const lineEnding = isUnix ? "\n" : "\r\n";
  fs.writeFileSync(profilePath, updatedProfile.join(lineEnding));
...
function enableAutoUse(useCdWrapper = false) {
  ...
  // Check shell type first (which is passed via --shell argument)
  if (shellType === "powershell" || shellType === "cmd") {
    enableAutoUsePowerShell(useCdWrapper);
  } else if (shellType === "bash" || shellType === "zsh") {

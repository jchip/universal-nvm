...
async function downloadNode(version, proxy, verifyssl) {
  ...
  for (const baseUrl of allDistUrls) {
    let url;
    try {
      url = common.nodejsDistUrl(`${version}/${common.makeNodeDistFileName(version)}`, baseUrl);
    } catch (err) {
      common.log(ck`<red>skipping invalid mirror <white>${baseUrl}</>: ${err.message}</>`);
      continue;
    }
    const options = {
      proxy,
    ...
    // A non-200 response (e.g. a 404 HTML page) may have been written to the
    // output path; remove it so it can't masquerade as a valid cached download
    // on a later run (which would then fail to extract until 'nvm cleanup').
    await fs.promises.rm(nodeCachePath, { force: true }).catch(() => {});
  }
...
async function doExtract(file, targetPath) {
  if (file.endsWith(".tgz")) {
    ...
    // node-stream-zip validates entry names (rejects traversal outside
    // targetPath) -- the zip source is user-configurable via
    // NVM_NODEJS_ORG_MIRROR, so hostile archives are in the threat model
...
async function install(targetPath, version, corepack) {
  ...
  try {
    ...
    try {
      await fs.promises.rm(path.join(targetPath, nodeFileName), { recursive: true, force: true });
    } catch (e) {
      //
    }
...
module.exports = {
  ...
  cmdInstall: async function(reqVersion, proxy, verifyssl, corepack) {
    ...
    if (!version) {
      common.log(
        ck`<red>Unable to find an exact node.js version that matches the requested version: '${common.ckEscape(reqVersion)}'</red>`
      );
      common.exit(1);
    }
    ...
    await fs.promises.mkdir(path.join(common.getNvmCacheDir(), version), { recursive: true });

    const status = await downloadNode(version, proxy, verifyssl);

    if (status !== true) {
      common.log(ck`<red>Failed to download and install Node.js <white>${version}</>.</>`);
      return common.exit(1);
    }
    ...
    const installed = await install(path.join(nodeDir, ".."), version, corepack);
    if (installed !== true) {
      return common.exit(1);
    }

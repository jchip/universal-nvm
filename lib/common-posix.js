...
function escapePath() {
  ...
  // Convert Windows paths to Unix-style for Git Bash on Windows
  // Check if we're in Git Bash/MSYS/Cygwin on Windows
  const isWindowsBash = process.platform === "win32" && process.env.MSYSTEM;

  if (isWindowsBash) {
    // Convert C:\path\to\dir to /c/path/to/dir
    pathStr = pathStr.split(path.delimiter).map(p => {
      // Check if it's a Windows absolute path (C:\... or C:/...)
      if (/^[A-Za-z]:[\\\/]/.test(p)) {
        // Convert C:\... to /c/...
        p = p.replace(/^([A-Za-z]):/, (match, drive) => `/${drive.toLowerCase()}`);

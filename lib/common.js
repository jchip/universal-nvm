...
const common = {
  ...
  // Does the path exist in its own right (symlink, file or dir)? Unlike _exists
  // (fs.access, which follows symlinks and so reports a dangling link as
  // missing), lstat inspects the link itself -- so a broken symlink is still
  // detected and can be unlinked before it collides with a new symlink (EEXIST).
  async lexists(pathName) {
    try {
  ...
  // Escape a string for interpolation into a chalker (ck) template. chalker
  // parses <...> as color markup and decodes HTML entities, so an untrusted
  // string (version file, package.json engines.node, CLI arg) interpolated
  // raw gets mangled -- e.g. ">=18.0.0" renders as "=18.0.0".
  ckEscape(str) {
    return String(str)
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
  ...
  async findLinkVersion() {
    ...
    if (link && (await this._exists(link))) {
      try {
        const match = (await fs.promises.readlink(link)).match(/(v[0-9]+\.[0-9]+\.[0-9]+)/);
        return match ? match[0] : undefined;
      } catch (e) {
        // link path exists but isn't a symlink (or is unreadable) - treat as not linked
      }
    }
  ...
  async findLocalVersions() {
    ...
    if ((await this._exists(nvmDir)) && (await fs.promises.lstat(nvmDir)).isDirectory()) {
      versions = this.sortVersions(
        (await fs.promises.readdir(nvmDir)).filter(x => x && x !== "bin" && x.startsWith("v"))
      );
    }
  ...
  isFullVersion(version) {
    // A concrete, fully-pinned version (x.y.z, optionally with a prerelease/build
    // tag). semver.valid returns null for ranges and partials ("20.10.x",
    // "^20.10.0", ">=18.0.0", "20", "20.10"), which must instead be resolved via
    // findVersionFromSemverRange -- counting dots can't tell those apart from x.y.z.
  ...
  getRemoteVersionsCachePath() {
    return path.join(this.getNvmCacheDir(), "remote-versions.json");
  },

  async getCachedRemoteVersions() {
    const cachePath = this.getRemoteVersionsCachePath();
  ...
  async saveRemoteVersionsCache(data) {
    const cachePath = this.getRemoteVersionsCachePath();
  ...
  async getRemoteFromJson(proxy, verifyssl, lts, useCache = true) {
    // Try to use cached data if requested
    if (useCache) {
      const cached = await this.getCachedRemoteVersions();
      if (cached) {
        let versions = lts ? cached.filter(x => x.lts) : cached;
    ...
    for (const baseUrl of allBaseUrls) {
      let url = baseUrl;
      try {
        url = this.nodejsDistUrl("index.json", baseUrl);
        if (failureErr) {
        ...
        if (resp.statusCode !== 200) {
          ...
          // Save the full response body to cache
          await this.saveRemoteVersionsCache(resp.body);
  ...
  async findVersionFromSemverRange(range, silent = false) {
    ...
    if (!silent) {
      this.log(ck`<red>No installed version satisfies requirement "${this.ckEscape(range)}"</>`);
      this.log(`Available versions: ${installedVersions.join(" ")}`);
      this.log(ck`<yellow>Hint: Try running <white>nvm install ${this.ckEscape(range)}</> first</>`);
      this.exit(1);
    }
    return null;
  },
  ...
  async getNodeVersionFromFile(silent = false) {
    ...
    for (const fileName of versionFileNames) {
      try {
        // These files are plain text from whatever editor the user has. Node's
        // "ascii" encoding masks the high bit off every byte rather than
        // validating, so a UTF-8 BOM (EF BB BF) silently decoded to "o;?" and
        // made a perfectly good "20.1.0" look like corrupt input. utf8 keeps the
        // BOM as U+FEFF, which .trim() removes (it is ECMAScript WhiteSpace)
        // along with any surrounding blank space.
        const rawSpec = (await fs.promises.readFile(fileName, "utf8")).trim();

        // An empty version file means "whatever is installed" -- the v1.11.1
        // behavior. semver already reads "" as the "*" range; normalize it up
        // front so no bare "" reaches the range resolver or an error message.
        const versionSpec = rawSpec || "*";

        // Security boundary: only accept a value that semver recognizes as a
        // valid version or range. This rejects arbitrary content (e.g. shell
        // metacharacters in a hostile .nvmrc / .node-version) before it can
        // flow into PATH/env script generation and get sourced -- see
        // createEnvironmentTmp + bin/unvm.sh and the auto-use error handshake.
        if (!semver.validRange(versionSpec)) {
          if (!silent) {
            this.log(ck`<red>Invalid version from ${fileName}: <white>${this.ckEscape(versionSpec)}</>.</>`);
            this.exit(1);
          }
          return null;
        }

        // Exact full version (x.y.z) - use as specified. semver.valid handles a
        // leading "v", so no need to test a "v"-prefixed variant separately.
        if (this.isFullVersion(versionSpec)) {
          return { version: versionSpec, source: fileName };
        }

        // Semver range or partial version (validated above) - resolve to the
        // highest installed match, or surface the range so the caller can show
        // a "not installed" hint.
    ...
    if (enginesNode) {
      ...
      if (this.isValidNodeVersion(enginesNode)) {
        ...
        if (!silent) {
          this.log(ck`<red>Invalid node version requirement in package.json: <white>${this.ckEscape(enginesNode)}</></>`);
          this.exit(1);
        }
        return null;
      }

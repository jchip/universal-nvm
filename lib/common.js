...
const common = {
  ...
  async getCachedRemoteVersions() {
    ...
    try {
      const stats = await opfs.stat(cachePath);
      const ageInMs = Date.now() - stats.mtimeMs;
      const oneHourInMs = 60 * 60 * 1000;

      // If cache is less than 1 hour old, use it
      if (ageInMs < oneHourInMs) {
        const cacheContent = await opfs.readFile(cachePath, "utf8");
  ...
  async saveRemoteVersionsCache(data) {
    ...
    try {
      if (!(await this._exists(cacheDir))) {
        await opfs.$.mkdirp(cacheDir);
      }
      await opfs.writeFile(cachePath, JSON.stringify(data, null, 2), "utf8");
    } catch (err) {

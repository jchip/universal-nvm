...
`;
  ...
  async createEnvironmentTmp(filePath, content) {
    ...
    // Write a temp file beside the target, then rename it into place -- the same
    // guarantee common-posix has. The shell wrapper reads this predictable path
    // right after we write it, so an in-place write let it see a half-written
    // script; and replacing the name (rather than writing through it) overwrites
    // a pre-planted file instead of following it. "wx" (O_CREAT|O_EXCL) makes the
    // temp itself unhijackable.
    //
    // No mode is set: Windows maps only the read-only attribute from POSIX bits,
    // so 0o600 buys nothing here -- the per-user TEMP dir is what limits access.
    //
    // this.rename, not fs.promises.rename: on Windows a scanner or the indexer
    // can hold a brief handle on a freshly created file and the rename fails
    // EPERM. That retry is exactly why common-win32 wraps rename at all, and a
    // temp-then-rename is precisely when the race shows up.
    ...
    try {
      ...
      await this.rename(tmpPath, filePath);
    } catch (err) {
